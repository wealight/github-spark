...
class KafkaManger(val zkConnect:String,val kafkaRootPath:String){
...
object KafkaManger{
  ...
  /**
    * 返回指定zookeeper 的root路径的znode信息 对应于kafka为broker
    * */
  def getBrokerList(zkConnect:String,kafkaRootPath:String) ={
    (new KafkaManger(zkConnect:String,kafkaRootPath:String)).getBrokerList()
  }
